let Canvas = {
    ...
    split: function (row, col, elements) {
        ...
        for (i = 0, offset = 0; i < row; ++i) {
            let list = [];
            for (j = 0; j < col; ++j) {
                list.push(echarts.init(elements[j + offset]));
            }
            this.viewports.push(list);
            offset += col;
        }
    },
    draw(gx, gy, option, merge) {
...
function draw(gx, gy, option, merge) {
...
function grid(row, col) {
    let body = empty();
    let rowH = (window.innerHeight - 5) / row;
    let colW = (window.innerWidth - 5) / col;
    let elements = [];
    for (i = 0; i < row; ++i) {
        let row = document.createElement("div");
        row.classList.add("row");
        row.style.cssText += "height: " + rowH + "px";
        for (j = 0; j < col; ++j) {
            let item = document.createElement("div");
            item.style.cssText += "width: " + colW + "px";
            row.appendChild(item);
            elements.push(item);
        }
        body.appendChild(row);
    }
    Canvas.split(row, col, elements);
}

// function test_heatmap(gx, gy) {
